// @ts-check
import { defineConfig } from 'astro/config';
import starlight from '@astrojs/starlight';

// The documentation site. It describes skill/ and is never held to it:
// `normative:` in CHANGEPACK.md names skill/ alone, so no page here is a spec.
export default defineConfig({
  // Where the site is meant to answer. GitHub Pages serves this repository
  // under a project path, so `base` carries that path into every generated
  // link and canonical URL. Publishing is a later package's business, and a
  // move to an apex domain is these two lines.
  site: 'https://allan-lancioni.github.io/changepack',
  base: '/changepack',
  integrations: [
    starlight({
      title: 'changepack',
      // One stylesheet, for layouts the built in components do not cover.
      customCss: ['./src/styles/custom.css'],
      // Both languages carry a prefix and neither sits at the root, so every
      // page has exactly one URL and no language is privileged. `root` is
      // unset on purpose: setting it would serve one locale from /.
      defaultLocale: 'en',
      locales: {
        en: { label: 'English', lang: 'en' },
        pt: { label: 'Português', lang: 'pt-BR' },
      },
      // Eight sections, each autogenerated from its own directory so a new
      // page appears by being written. Order inside a group is `sidebar.order`
      // in the page frontmatter, because alphabetical is not the reading
      // order anywhere here.
      sidebar: [
        {
          label: 'Start',
          translations: { 'pt-BR': 'Começo' },
          items: [{ autogenerate: { directory: 'start' } }],
        },
        {
          label: 'The project file',
          translations: { 'pt-BR': 'O arquivo do projeto' },
          items: [{ autogenerate: { directory: 'project-file' } }],
        },
        {
          label: 'Routes',
          translations: { 'pt-BR': 'Rotas' },
          items: [{ autogenerate: { directory: 'routes' } }],
        },
        {
          label: 'The package',
          translations: { 'pt-BR': 'O pacote' },
          items: [{ autogenerate: { directory: 'package' } }],
        },
        {
          label: 'Git',
          translations: { 'pt-BR': 'Git' },
          items: [{ autogenerate: { directory: 'git' } }],
        },
        {
          label: 'Versions',
          translations: { 'pt-BR': 'Versões' },
          items: [{ autogenerate: { directory: 'versions' } }],
        },
        {
          label: 'Rationale',
          translations: { 'pt-BR': 'Fundamento' },
          items: [{ autogenerate: { directory: 'rationale' } }],
        },
        {
          label: 'Internals',
          translations: { 'pt-BR': 'Interno' },
          items: [{ autogenerate: { directory: 'internals' } }],
        },
      ],
      social: [
        {
          icon: 'github',
          label: 'GitHub',
          href: 'https://github.com/allan-lancioni/changepack',
        },
      ],
    }),
  ],
});
